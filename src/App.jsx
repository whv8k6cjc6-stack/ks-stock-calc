...
const accVal=acc=>{
  if(acc.type==="cash")return pN(acc.cash);
  const fx=acc.type==="overseas"?(pN(acc.fx)||1):1;
  const mult=acc.type==="stock"?1000:1;
  return acc.holdings.reduce((s,h)=>s+pN(h.qty)*pN(h.price)*mult*fx,0);
};
...
function ScCalc({S}){
  const c=pN(S.cap);
  const validPos=S.positions.map(p=>{const pr=ALL.find(x=>x.i===p.pid);const x=pN(p.price),n=parseInt(p.lots)||0;if(!pr||x<=0||n<=0)return null;return{pr,x,n};}).filter(Boolean);
  const res=useMemo(()=>{if(!validPos.length||c<=0)return[];return SCO.map(sc=>{let tPnl=0,nTP=0,nTM=0;validPos.forEach(({pr,x,n})=>{const nx=x*(1+sc);tPnl+=(nx-x)*(pr.cat==="index"?pr.mult:pr.spl)*n;nTP+=lv(pr,nx)*n;nTM+=om(pr,nx)*n;});const nC=c+tPnl,nLv=nC>0?nTP/nC:0,ri=nC>0&&nTM>0?nC/nTM*100:0;return{sc,tPnl,nC,nLv,ri};});},[validPos,c]);
  return(<div>
    <Card><SL>持倉（自動帶入「期貨」分頁）</SL>
      {validPos.length===0?<div style={{textAlign:"center",fontSize:12,color:C.dim,padding:"4px 0"}}>無持倉</div>:
        validPos.map(({pr,x,n},i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",padding:"8px 12px",background:"rgba(255,255,255,0.03)",borderRadius:8,marginBottom:6,fontSize:13}}><span style={{fontWeight:600,color:CC[pr.cat]||C.blue}}>{pr.sn}</span><span style={{color:C.muted}}>{n}口 @{x}</span></div>))}
    </Card>
...
function FutTab({S,U}){
  ...
  return(<div>
    ...
    <Card><SL>期貨持倉（自動儲存）</SL>
      ...
      <AddBtn onClick={()=>U(s=>({...s,positions:[...s.positions,{id:s.nid,pid:"",price:"",lots:""}],nid:s.nid+1}))}/>
    </Card>
...
function AssetTab({S,U}){
  ...
  const settle=useMemo(()=>{
    ...
    t+=`────────────\n`;
    t+=`期貨權益　 ${fM(futEq)}\n`;
    accVals.forEach(a=>{if(a.val>0)t+=`${a.name}　 ${fM(a.val)}\n`;});
    t+=`────────────\n`;
    ...
    return t;
  },[futEq,accVals,totalAssets,futPos,secVal,totalPos,totalLev,futRi]);

  const saveSnapshot=()=>{
    const snap={date:today(),totalAssets,totalPos,lev:totalLev,futEq,secVal,cashVal,futPos,futRi};
    U(s=>{
  ...
  return(<div>
        ...
        <Card key={a.id} ac={at.color}>
          ...
          {a.type==="cash"?(
            ...
            <>
              ...
              {a.holdings.map(h=>(
                <div key={h.id} style={{display:"grid",gridTemplateColumns:"1.3fr 0.8fr 0.9fr auto",gap:6,marginBottom:7,alignItems:"center"}}>
                  <input value={h.name} onChange={e=>updAcc(a.id,x=>({...x,holdings:x.holdings.map(y=>y.id===h.id?{...y,name:e.target.value}:y)}))} placeholder="名稱" style={{...bI,padding:"8px 9px",fontSize:13}}/>
                  <input type="number" inputMode="decimal" value={h.qty} onChange={e=>updAcc(a.id,x=>({...x,holdings:x.holdings.map(y=>y.id===h.id?{...y,qty:e.target.value}:y)}))} placeholder={at.unit} style={{...bI,padding:"8px 9px",fontSize:13}}/>
                  <input type="number" inputMode="decimal" value={h.price} onChange={e=>updAcc(a.id,x=>({...x,holdings:x.holdings.map(y=>y.id===h.id?{...y,price:e.target.value}:y)}))} placeholder={a.type==="fund"?"淨值":"價格"} style={{...bI,padding:"8px 9px",fontSize:13}}/>
          ...
          {a.val>0&&<div style={{marginTop:10,fontSize:13,textAlign:"right",color:at.color,fontWeight:700}}>小計 {fM(a.val)} 元</div>}
        </Card>
      );
    })}
    ...
    <Card ac={C.green}>
      ...
      {[
        ...
        ...accVals.filter(a=>a.val>0).map(a=>({label:a.name,val:a.val,c:ACC_TYPES[a.type].color})),
      ].map(({label,val,c:cc})=>(
      ...
      <div style={{marginTop:10,padding:"10px 12px",background:"rgba(0,0,0,0.2)",borderRadius:8,fontSize:13,display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
        ...
        <span style={{color:C.muted}}>證券部位 <b style={{color:C.text}}>{fM(secVal)}</b></span>
        <span style={{color:C.muted}}>總部位 <b style={{color:C.text}}>{fM(totalPos)}</b></span>
...
function TrendTab({S,U}){
  ...
  const csv=useMemo(()=>{
    let t="日期,總淨資產,總部位,總槓桿,期貨權益,證券部位,現金,期貨風險指標\n";
    snaps.forEach(s=>{t+=`${s.date},${Math.round(s.totalAssets)},${Math.round(s.totalPos)},${s.lev.toFixed(2)},${Math.round(s.futEq)},${Math.round(s.secVal)},${Math.round(s.cashVal||0)},${s.futRi?Math.round(s.futRi):""}\n`;});
    return t;
  },[snaps]);
  return(<div>
...
const INIT={
  ...
  positions:[{id:1,pid:"",price:"",lots:""}],nid:2,
